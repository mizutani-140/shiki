...
### 0. チーム作成（セッション開始直後 — 必須）

**セッション開始時に必ず `TeamCreate` でチームを作成する。**
GOAL.md の内容に基づいて必要な役割を判断し、メンバーを編成する。

#### エンジン割当ルール（厳守）

| タスク種別 | 担当 | Teams での役割例 |
|-----------|------|-----------------|
| 関数実装 | **Codex** | implementer |
| テスト生成 | **Codex** | tester |
| コードレビュー | **Codex** | reviewer |
| 定型コード | **Codex** | implementer |
| CI 修復 | **Codex** | ci-fixer |
...
1. `GOAL.md` を読み、プロジェクトに必要な作業種別を洗い出す
2. 上記ルールに基づいて、Codex 担当と CC 担当を分類する
3. `TeamCreate` でチームを作成する
4. Codex 担当タスクには `engine: codex` を設定する
5. CC 担当タスクには `engine: claude-team` を設定する

### 1. 計画策定（θ₁ UNDERSTAND → θ₂ GENERATE）
- `GOAL.md` を読み、ユーザーのゴールと受け入れ条件を理解する
- `roles/roles.yaml` を参照し、authority_layers に基づいてチーム編成を確認する
...
- **エンジン振分**: 上記ルールに従い、各タスクに `engine` フィールドを設定する

### 3. 実行管理（θ₄ EXECUTE — Dual Engine）

#### engine=codex のタスク（関数実装・テスト生成・定型コード・CI修復）
- executor メンバーに「Codex MCP 経由で委託」を指示する
- executor が exec verify を実行して受け入れ条件を検証する
- Codex 失敗時は CC executor が実装にフォールバックする

#### engine=claude-team のタスク（設計判断・デバッグ・リファクタリング）
- CC executor メンバーに直接実装を指示する

- メンバーのPlan提出を確認し、承認/差し戻しを行う
- `ExitPlanMode` でメンバーをStandard Modeに移行させる
- タスク進捗を `TaskList` で定期的に確認する
- ブロッキング問題の解決を調整する

### 4. レビュー（θ₅ VERIFY — Codex が担当）

**レビューは必ず Codex に委託する。CC はレビュー結果を確認し、マージ判断のみ行う。**

1. 実装完了したタスクを Codex MCP にレビュー委託する
2. Codex のレビュー結果を確認する
3. 指摘事項があれば修正タスクを作成する
4. 全タスクのレビューが完了したら θ₆ に進む

### 5. 統合（θ₆ INTEGRATE）
- 全タスクの実装とレビューが完了していることを確認する
- マージ判断を行う（CC の責務）
- 統合作業を調整する
