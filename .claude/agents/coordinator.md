...
- **エンジン振分**: `python3 scripts/engine_router.py --all` でタスクに最適エンジンを自動割当する

### 3. 実行管理（θ₄ EXECUTE — Dual Engine）

タスクのエンジン割当に応じて実行方法が異なる：

#### engine=codex のタスク
- executor メンバーに「Codex MCP 経由で委託」を指示する
- executor が Codex の出力を検証し、exec verify を実行する
- Codex 失敗時は executor 自身が実装にフォールバックする

#### engine=claude-team のタスク
- executor メンバーに直接実装を指示する（従来通り）
- 失敗時は Codex MCP へのフォールバックを検討する
...
| テスト生成 | Codex | パターン生成 |
| 複数ファイル横断 | Claude | 文脈理解が必要 |
| リファクタリング | Claude | 判断が必要 |
