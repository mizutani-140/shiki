...
5. **`TeamCreate` でチームを作成する** — GOAL に基づいて必要な役割を判断し、メンバーを編成する（CLAUDE.md Section 5 参照）
6. 現在のθフェーズを特定する
7. 報告する: 「セッション初期化完了。モード: X。θフェーズ: Y。チーム: N名。アクティブタスク: N件」
...
- 関数実装（単一/複数ファイル問わず、仕様が明確なもの）
- テストコード生成（ユニットテスト、統合テスト）
- 定型コード・ボイラープレート生成
- lint / typecheck エラーの自動修正（CI 修復）
- **コードレビュー**（θ₅ VERIFY で必ず Codex にレビューを委託する）

### CC が行うタスク

以下は Claude Code（CC）が直接行う：
- 設計・アーキテクチャ判断
- デバッグ（原因調査が必要なもの）
- リファクタリング
- 計画・タスク分解
- マージ判断（Codex のレビュー結果を確認した上で CC が判断する）

### レビューの委託手順（θ₅ VERIFY）

**レビューは必ず Codex に委託する。** CC はレビュー結果に基づきマージ判断のみ行う。
