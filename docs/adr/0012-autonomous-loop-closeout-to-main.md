...
Re-entrancy: `create_closeout_pr` is gated on `not task.closeout_pr` and the closeout branch name is deterministic; if a closeout PR already exists for the branch it is adopted rather than duplicated.
